...
import pandas as pd
from wattle import Node, Branch, Tree, Split_Test
import unittest
...
class test_split_functions(unittest.TestCase):
  def test_gain_ratio(self):
    ...
    test = gain_ratio_split(Node(data=dataframe, class_attribute='Defective',
      positive_class='1'), 2)
  ...
  def test_cost_reduction(self):
    ...
    # numerical attribute.
    dataframe = pd.read_csv('data/LOC_SDP.csv')
    node = Node(data=dataframe, class_attribute='Defective',
      positive_class='1')
